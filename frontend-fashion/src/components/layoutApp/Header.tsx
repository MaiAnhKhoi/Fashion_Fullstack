...
import { useShopSetting } from "@/hooks/queries/useShopSetting";
...
export default function Header({
  ...
  const { openModelLogin, openModelCart, openModelSearch } = useUIStore();
  const accessToken = useAuthStore((s) => s.accessToken);
  const navigate = useNavigate();

  // Đã đăng nhập -> vào trang account; chưa thì mở modal login.
  ...
  return (
    <>
      <header id="header" className={parentClass}>
        <div className={fullWidth ? "container-full" : "container"}>
          <div className="row wrapper-header align-items-center">
            ...
            <div className="col-xl-2 col-lg-2 col-md-3 col-6 p-4">
      ...
      {/* 
      <ChatSupportModal isOpen={chatOpen} onClose={() => setChatOpen(false)} /> */}
    </>
  );
}
